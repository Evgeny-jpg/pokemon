...
echo '<img src="image.jpg" width="360px>';
?>
<?php
...
$res = mysqli_query($link, "SELECT `visit_id` FROM `visits` WHERE `date`='$date'"); 
...
mysqli_query($link, "INSERT INTO `ips` SET `ip_address`='$visitor_ip'");
...
// Если такой IP-адрес уже сегодня был (т.е. это не уникальный посетитель)
    if (mysqli_num_rows($current_ip) == 1)
    {
        ...
        mysqli_query($link, "UPDATE `visits` SET `views`=`views`+1 WHERE `date`='$date'");
    }
    ...
    else
    {
        ...
        mysqli_query($link, "UPDATE `visits` SET `hosts`=`hosts`+1,`views`=`views`+1 WHERE `date`='$date'");
    }
